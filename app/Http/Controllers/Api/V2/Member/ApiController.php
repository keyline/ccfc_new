...
class ApiController extends Controller
{
                        ...
                        if ($transactions) {
                            ...
                            foreach ($transactions as $transaction) {
                                ...
                                if (
                                    ...
                                    !empty(SearchInvoicePdf::getSummaryBillLinkApp(
                                        $checkUser->user_code,
                                        $transaction['Month']
                                    ))
                                ) {
                                ...
                                $detailed_bill_link = '';
                                if (
                                    SearchInvoicePdf::isBillUploaded(implode("_", explode(
                                        " ",
                                        $transaction['Month']
                                    ))) &&
                                    !empty(SearchInvoicePdf::getDetailBillLinkApp(
                                        $checkUser->user_code,
                                        $transaction['Month']
                                    ))
                                ) {
    ...
    public function makeCurlRequest($url, $postData = [])
    {
        ...
        // );
        $headers = array(
            "Authorization: Bearer ZWfkQXe7s3UEv8sAndwtGVZWfPR7YDgN18v7t6rAw5AFRs67F1N2BlsusJR5HaOc9CRbFjy4e_WF80truA06siBF_8cqE44-chkiHaCEkXOlXeJYYbXiF3Zu_umiG94C5tzwuflBsrXnTAqAvlnG8RYfi2p_3qaLGycnTtxDiAXP9nRftIntfWe4A9H2GHkYi7U6ia4919nKjSdtx2vjNKwSnoJ4e11s6KEcIoDvMmTL1905hnGLFyDfJIe4nNE3wX4tT4VFmb8gYT8iU3nvxTmHZdcUDfbTg10kVAfcfrQ",
            "Content-Type: application/json",
            "Content-Length: 0",
        );
    ...
    public function razorpayResponse(Request $request)
    {
        ...
        try {
            // ✅ Header auth check
            $headerData = $request->header();
            if (!isset($headerData['key'][0]) || $headerData['key'][0] !== $project_key) {
                $apiStatus = false;
                $apiMessage = 'Unauthenticated Request !!!';
                return $this->response_to_json($apiStatus, $apiMessage, []);
            }

            $app_access_token = $headerData['authorization'][0] ?? null;
            $getTokenValue = $this->tokenAuth($app_access_token);
            ...
            if (!$checkUser || !in_array($checkUser->status, ['ACTIVE', 'INACTIVE'])) {
                $apiStatus = false;
                $apiMessage = 'Your Account Is Not Active Yet !!!';
                return $this->response_to_json($apiStatus, $apiMessage, []);
            }
            // Decode JSON input
            $json = file_get_contents('php://input');
            $this->isJSON($json);
            $requestData = $this->extract_json($json);

            // ✅ Extract 'data' block
            $data = $requestData['data'] ?? null;
            if(!isset($requestData['amount'])){
                $apiStatus = false;
                $apiMessage = 'Amount is Not Present !!!';
                return $this->response_to_json($apiStatus, $apiMessage, []);
            }
            if ($requestData['status'] == 'success') {                
                $requiredFields = ['razorpay_payment_id', 'razorpay_order_id', 'razorpay_signature'];
                if (!$this->validateArray($requiredFields, $data)) {
                    $apiStatus = false;
                    $apiMessage = 'All Data Are Not Present !!!';
                    return $this->response_to_json($apiStatus, $apiMessage, []);
                }
                ...
                $get_amount = $requestData['amount'];
                if ($get_amount !== $amount) {
                    $apiStatus = false;
                    $apiMessage = 'Amount is not same!';
                    $apiResponse = [
                        'payment_amount' => $amount,
                ...
                $checkTxn = DB::table('payu_transactions')->where('transaction_id', $payment_id)->count();
                if ($checkTxn > 0) {
                    $apiStatus = true;
                    $apiMessage = 'Transaction Already Exists.';
                    return $this->response_to_json($apiStatus, $apiMessage, []);
                }
                ...
                $postData = [
                    ...
                    'gateway'           => 'Razorpay',
                    'body'              => '',
                    'destination'       => 'https://ccfc1792.com/member/payment/razorpaycallback',
                    'hash'              => $signature,
                ...
                $generalSettings = GeneralSetting::find(1);
                $subject = $generalSettings->site_name . ' :: Razorpay Payment Success';
                $message = view('email-templates.payment-success', $mailData);
                ...
                $apiExtraField = 'response_code';
                $apiExtraData = http_response_code(200);

                return $this->response_to_json($apiStatus, $apiMessage, $apiResponse);
            } else {

                // echo 'Razorpay Payment Failed: ' . json_encode($data);
                // ✅ Insert into DB
                $postData = [
                    'paid_for_id'       => $uId,
                    'paid_for_type'     => 'App\Models\User',
                    'transaction_id'    => $uId . '-failed-' . time(), // Unique transaction ID
                    'gateway'           => 'Razorpay',
                    'body'              => '',
                    'destination'       => 'https://ccfc1792.com/member/payment/razorpaycallback',
                    'hash'              => '',
                ...
                // $generalSettings = GeneralSetting::find(1);
                // $subject = $generalSettings->site_name . ' :: Razorpay Payment Failed';
                // $message = view('email-templates.payment-success', $mailData);
            ...
            $apiStatus = false;
            $apiMessage = 'Server Error: ' . $e->getMessage();
        ...
        if ($headerData['key'][0] == $project_key) {
            ...
            if ($getTokenValue['status']) {
                ...
                if ($checkUser) {
                    if ($checkUser->status == 'ACTIVE' || $checkUser->status == 'INACTIVE') {
                        /* push notification */
                        $getUserFCMTokens   = UserDevice::select('fcm_token')->where('fcm_token', '=', 'cjwWqtek8E1yviXPwOiBpE:APA91bGwKYoZeEHxbJ4t7wJNjGuHodq0MYanQozBNLrtRheKcsHxT5b3lSXLjG7O2phdnRImPOpLP1Jqlr1tRYFzL4sAZ5xUp_2G9-iSDT_TyN3xoSWye8Y')->get();
