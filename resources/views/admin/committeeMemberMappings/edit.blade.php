...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST"
            action="{{ route("admin.committee-member-mappings.update", [$committeeMemberMapping->id]) }}"
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('committee') ? 'is-invalid' : '' }}"
                    ...
                    <option value="{{ $id }}"
                        {{ (old('committee_id') ? old('committee_id') : $committeeMemberMapping->committee->id ?? '') == $id ? 'selected' : '' }}>
                        {{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
               <!-- <label class="required"
                ...
                <select class="form-control select2 {{ $errors->has('member') ? 'is-invalid' : '' }}" name="member_id"
                    ...
                    <option value="{{ $id }}"
                        {{ (old('member_id') ? old('member_id') : $committeeMemberMapping->member->id ?? '') == $id ? 'selected' : '' }}>
                        {{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('designation') ? 'is-invalid' : '' }}" type="text"
                    ...
                    value="{{ old('designation', $committeeMemberMapping->designation) }}" required>
            ...
            <div class="form-group">
                <label class="required"
                    for="member_order">Sort Order</label>
                <input class="form-control {{ $errors->has('member_order') ? 'is-invalid' : '' }}" type="text"
                    name="member_order" id="member_order"
                    value="{{ old('member_order', $committeeMemberMapping->member_order) }}" required>
                @if($errors->has('member_order'))
                <span class="text-danger">{{ $errors->first('member_order') }}</span>
                @endif
                <!-- <span class="help-block">{{ trans('cruds.committeeMemberMapping.fields.member_order_helper') }}</span> -->
            </div>
            <div class="form-group">
