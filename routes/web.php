...
Route::get('/general_committee', function () {
    ...
    $committeeMemberMappings = CommitteeMemberMapping::join('committee_names', 'committee_names.id', '=', 'committee_member_mappings.committee_id')
                            ...
                            //->orderByRaw('committee_member_mappings.member_order IS NULL, committee_member_mappings.member_order', 'ASC')
                            ->orderByRaw('committee_member_mappings.member_order IS NULL ASC')
                            ->orderBy('committee_member_mappings.member_order', 'ASC')
                            ->get();
    /*dd(CommitteeMemberMapping::join('committee_names', 'committee_names.id', '=', 'committee_member_mappings.committee_id')
                                ->join('users', 'users.id', '=', 'committee_member_mappings.member_id')
                                ->where('committee_names.id', '=', '1')
                                ->orderByRaw('committee_member_mappings.member_order IS NULL, committee_member_mappings.member_order', 'ASC')
                                ->toSql());*/
...
Route::group([
    ...
    Route::post('payment/razorpaycallback', ['as' => 'razorpaycallback', 'uses' => 'PaymentController@razorpaycallback']);
...
Route::get('tenders', function () {
    ...
    $uploadedTenders = DocumentOrganizer::whereIn('cdo_id', [1, 2, 3, 4, 5, 6, 7])->documents()->where('ctd_archive_status', '1')
                        ->orderBy('ctd_created_at', 'desc')
                        ->get();
